                    ...
                    <div class="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                        @foreach ($services as $service)
                            <label class="flex items-center gap-2 rounded-md border border-gray-200 p-3 text-sm text-gray-700 transition hover:border-indigo-300 hover:bg-indigo-50">
                                <input type="checkbox" name="services[]" value="{{ $service->id }}" @checked(in_array($service->id, $selectedServices)) class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" />
                                {{ $service->name }}
                            </label>
                        @endforeach
                    </div>
